...
def commit_import(conn, frame, category, digest, filename, uploaded_on, login_id, kind):
    ...
    with conn:
        ...
        conn.executemany(f"INSERT INTO rework_items ({','.join(cols)}) VALUES ({','.join('?' for _ in cols)})", records)
        conn.execute('INSERT INTO import_batches VALUES (?,?,?,?,?)', (digest, category, filename, uploaded_on.isoformat(), len(records)))
        conn.execute("INSERT INTO upload_logs(category,uploaded_by,file_name,row_count,status,note) VALUES(?,?,?,?,'확정',?)", (category, login_id, filename, len(records), f'{kind}; sha256={digest}'))
    return len(records)
